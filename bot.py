...
def default_data():
    return {
        "channels": [],       # ايديات القنوات المشتركة
        "groups": [],         # ايديات المجموعات
        "from": [],           # ايديات الأشخاص (المستخدمين)
...
# =========================================================
#   EXCHANGE LOGIC & LIVE COUNTDOWN (UPDATE EVERY MINUTE)
# =========================================================
...
def run_exchange_tick(data):
    ...
    for ad in data.get("ads", []):
        ...
        if remaining_sec <= 0:
            ...
            remaining_minutes = int(ceil_div(remaining_sec, 60))
            
            # نص الإعلان مع العد التنازلي المتجدد
...
def process_update(data, update):
    # 1. معالجة الإضافات أو التغييرات في حالة البوت داخل القنوات/المجموعات
    ...
    # 3. معالجة الرسائل العادية وحفظ المستخدمين في مصفوفة from
    if "message" in update:
        ...
        if chat_type == "private":
            if user_id not in data["from"]:
                data["from"].append(user_id)
                save_data(data)

            if str(user_id) == str(ADMIN_ID) and text == "/start":
...
def main():
    print("Bot is running with getUpdates and live countdown...")
    offset = 0
    data = load_data()
    ...
    while True:
        try:
            ...
            # تشغيل فحص وإحداثيات العد التنازلي وتحديث المنشورات كل حلقة (تتحكم دقيقة بدقيقة)
            if data.get("running", True):
                run_exchange_tick(data)
                
            time.sleep(60) # يتم الفحص والتحديث كل 60 ثانية تماماً
        except Exception as e:
